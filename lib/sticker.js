...
const stickerlight = (imageUrl) => new Promise((resolve, reject) => {
    fetch(`https://api.vhtear.com/lightning?link=${encodeURIComponent(imageUrl)}&apikey=ANTIGRATISNIHANJENKKK`, {
        method: 'GET',
    })
    .then(async res => {
...
const stickerburn = (imageUrl) => new Promise((resolve, reject) => {
    fetch(`https://api.vhtear.com/burning_fire?link=${encodeURIComponent(imageUrl)}&apikey=ANTIGRATISNIHANJENKKK`, {
        method: 'GET',
    })
    .then(async res => {
        const text = await res.json()
