...
INGAT JANGAN JUAL SCRIPT HNZM KEPADA ORANG LAIN!
INGIN PREMIUM? CHAT NABIL!
...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6281281524356@c.us'
        ...
        function cekumur(obj){
            if (obj === true){
                ...
                return tobz.reply(from, `Kamu belum cukup umur untuk menggunakan BOT HNZM, min 16 tahun\n\nKamu bisa mendaftar ulang dengan cara donasi terlebih dahulu, bales ${prefix}donasi\nHubungi Owner : wa.me/6281281524356`, id) //if user is not registered
            }
        }
        ...
        // FUNCTION
                ...
                for (let i of stickerspam){
                    if(i.id === id){
                        if (i.msg >= 12) {
                            found === true 
                            tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh HNZM`, id).then(() => {
                                tobz.removeParticipant(groupId, id)
                            })
                            return true;
                        }else if(i.msg >= 12){
                            found === true
                            tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu terdeteksi spam sticker!\nMohon tidak spam 5 sticker lagi atau nomor akan di kick oleh HNZM!`, id)
                            return true
                        }else{
                            found === true
                            return false;
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 1) { // 1X BADWORD AKAN TERKENA KICK
                        kasar === true 
                        tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nPeringatan! 2x lagi berkata kasar, HNZM akan kick kamu dari grup!`, id)
                        }
                    if (msg >= 3) { // 3X BADWORD AKAN TERKENA KICK
                        kasar === true 
                        tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh HNZM!`, id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        })
                        return true;
                    }else{
        ...
        function GenerateSerialNumber(mask){
            ...
            return serialNumber;
                ...
                if(body === prefix+'mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin HNZM!', id)
                ...
                if (body === prefix+'unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner HNZM!', id)
        ...
        case prefix+'banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner HNZM!', id)
        ...
        case prefix+'ttp2':
            ...
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#ttp2 [ Teks ]*, contoh *#ttp2 HNZM*`, id)
        ...
        case prefix+'blackpink':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#blackpink [ Teks ]*, contoh *#blackpink HNZM*`, id)
            tobz.reply(from, mess.wait, id)
            const blpk = body.slice(11)
            if (blpk.length > 10) return tobz.reply(from, '*Teks Terlalu Panjang!*\n_Maksimal 10 huruf!_', id)
            await tobz.sendFileFromUrl(from, `https://api.vhtear.com/blackpinkicon?text=${blpk}&apikey=ANTIGRATISNIHANJENKKK`, 'blackpink.jpg', '', id)
            break
		case prefix+'infoalamat':
                ...
                if (args.length === 1) return tobz.reply(from, 'Kirim perintah *#heroml [nama hero]*Contoh : *#heroml akai*', id)
                try {
...
\n➸ Deskripsi : ${resp.data.result.deskripsi}`
            ...
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
                        ...
                        await limitAdd(serial)
                        if (args.length === 1)  return tobz.reply(from, 'Kirim perintah #infomotor merk\nContoh #infomotor beat', id)
                        arg = body.trim().split(' ')
                        console.log(...arg[1])
                        var slicedArgs = Array.prototype.slice.call(arg, 1);
                        console.log(slicedArgs)
                        const istalks = await slicedArgs.join(' ')
                        console.log(istalks)
                        try {
                        const istalk2s = await axios.get('https://api.vhtear.com/infomotor?merk=' + istalks + '&apikey=' + vhtearkey)
            ...
            \n➸ Spek : ${spesifikasi}`
                        ...
                        tobz.sendImage(from, base64, image, istalk3s)
                        } catch (err) {
                         console.error(err.message)
                         await tobz.sendFileFromUrl(from, errorurl2, 'error.png', '💔 Maaf, Data tidak ditemukan')
                         tobz.sendText(ownerNumber, 'infomotor Error : ' + err)
                       }
                      break
                    case prefix+'infomobil':
                        ...
                        await limitAdd(serial)
                        if (args.length === 1)  return tobz.reply(from, 'Kirim perintah #infomotor merk\nContoh #infomotor avanza', id)
                        arg = body.trim().split(' ')
                        console.log(...arg[1])
                        var slicedArgs = Array.prototype.slice.call(arg, 1);
                        console.log(slicedArgs)
                        const istalkss = await slicedArgs.join(' ')
                        console.log(istalkss)
                        try {
                        const istalk2ss = await axios.get('https://api.vhtear.com/infomobil?merk=' + istalkss + '&apikey=' + vhtearkey)
            ...
            \n➸ Spek : ${spesifikasi}`
                        ...
                        const pictk = await bent("buffer")(image)
                        const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
                        tobz.sendImage(from, base64, image, istalk3ss)
                        } catch (err) {
                         console.error(err.message)
                         await tobz.sendFileFromUrl(from, errorurl2, 'error.png', '💔 Maaf, Data tidak ditemukan')
                         tobz.sendText(ownerNumber, 'infomobil Error : ' + err)
                       }
                      break
        case prefix+'thunder':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#thunder [ Teks ]*, contoh *#thunder Mantan*`, id)
            tobz.reply(from, mess.wait, id)
            const thndr = body.slice(9)
            if (thndr.length > 10) return tobz.reply(from, '*Teks Terlalu Panjang!*\n_Maksimal 10 huruf!_', id)
            await tobz.sendFileFromUrl(from, `https://api.vhtear.com/thundertext?text=${thndr}&apikey=ANTIGRATISNIHANJENKKK`, 'thndr.jpg', '', id)
            break
            case prefix+'galaxy':
                if(isReg(obj)) return
                ...
                await tobz.sendFileFromUrl(from, `https://api.vhtear.com/galaxytext?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
		...
		case prefix+'phub':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Nabil|Dev HNZM*`, id)
            ...
            if (argz.length >= 2) {
                ...
                tobz.sendFileFromUrl(from, `https://api.vhtear.com/pornlogo?text1=${lpornhub}&text2=${lpornhub2}&apikey=ANTIGRATISNIHANJENKKK`)
                await limitAdd(serial)
            } else {
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Nabil|Dev HNZM*`, id)
            }
            break
            ...
            case prefix+'party':
                ...
                case `${prefix}lovemaker`:
                    case `${prefix}textlove`:
                       //if (!isVipUser) return tobz.reply(from, 'Fitur Ini Khusus VIP!\n Daftar VIP Hanya 5k/Bulan\n\n*Mau Daftar VIP?*\nChat Owner BOT\n_wa.me/6281220439155_ ', id)
                        //if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                   ...
                   case `${prefix}glitchtext`: 
                    case `${prefix}glitchteks`:
                       ...
                       if (argz.length >= 3) {
                           ...
                           await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah #glitchteks  |Teks1|Teks2, contoh #glitchteks |Nabil|Bot`, id)
                       }
                       break
		case prefix+'silktext':
		     if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
			 if (args.length === 1) return tobz.reply(from, `Kirim perintah *#silktext [ Teks ]*, contoh *#silktext HNZM*`, id)
		     const silk = body.slice(10)
			 await limitAdd(serial)
			 tobz.sendFileFromUrl(from, `https://api.vhtear.com/silktext?text=${silk}&apikey=ANTIGRATISNIHANJENKKK`,'silk.jpg','*Bamru Kemren*', id)
			 break
		case prefix+'st':
		      if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
			  if (args.length === 1) return tobz.reply(from, `Kirim perintah *#st [ Teks ]*, contoh *#st HNZM Ganss Anjay*`, id)
		      const st = body.slice(4)
			  await limitAdd(serial)
		      tobz.sendFileFromUrl(from, `https://api.vhtear.com/slidingtext?text=${st}&apikey=ANTIGRATISNIHANJENKKK`,'titit.mp4','*Done*', id)
			  break
		case prefix+'missing':
            if ((isMedia || isQuotedImage)) {
            const encryptMedia = isQuotedImage ? quotedMsg : message
            const mediaData = await decryptMedia(encryptMedia, uaOverride)
            const urlimek = await uploadImages(mediaData, false)   
            const missings = body.slice(9)
            const missings1 = missings.split('|')[0]
            const missings2 = missings.split('|')[1]
            const missings3 = missings.split('|')[2]
            const resp = await axios.get(`https://api.vhtear.com/missingperson?text1=${missings1}&text2=${missings2}&text3=${missings3}&link=${urlimek}&apikey=ANTIGRATISNIHANJENKKK`)
            if (resp.data.error) return tobz.reply(from, resp.data.error, id)
            const anm2 = `${resp.data.result.imgUrl}`
            tobz.reply(from, mess.wait, id)
            tobz.sendFileFromUrl(from, anm2, 'missing.jpg', `Missing`, id)
            }
            break
            case prefix+'daftar':  // NAMBAHIN NOMOR DI DATABASE
                ...
                const umurnye = argz[2]
                    ...
                    if(umurnye >= 40) return await tobz.reply(from, 'Kamu terlalu tua, kembali lagi ke masa muda untuk menggunakan HNZM', id)
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
                ...
                case prefix+'daftarulang':
                    if (!isAdmin) return tobz.reply(from, 'Command ini hanya dapat digunakan oleh admin HNZM', id)  
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
        ...
        case prefix+`vipstatus`:
            ...
            tobz.sendFileFromUrl(from, pfpp, 'photo.jpg', `
──────────────────
        『 𝐕𝐈𝐏 USER 』
• Name      : ${pushname}
...
${desc}`)
        ...
        case prefix+'owner':
        case prefix+'creator':
            tobz.sendContact(chatId, `6281281524356@c.us`)
            tobz.reply(from, 'Itu nomor Pacar ku, eh eh maksudnya Owner ku', id)
            break
        ...
        case prefix+'simi':
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin HNZM!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
...
🌐️ *URL*: ${url}`
        ...
        case prefix+'translate':
            ...
            if(args.length >= 2){
                var codelang = args[1]
                var teks = body.slice(11+codelang.length);
        ...
        case prefix+'nhentai': // SEARCH NHENTAI
            ...
            if (!isNsfw) return tobz.reply(from, 'command/Perintah NSFW belum di aktifkan di group ini!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
			if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin HNZM!`, id)
           ...
           try {
                if (quotedMsg && quotedMsg.type == 'image') {
                    ...
                    if (doujinveza.error) {
                        ...
                        tobz.sendFileFromUrl(from, shortenernh, `${title}.zip`, '_NHENTAI DOWNLOADER BY TOBZ_', id)
                        } catch (err){
                            console.log(err)
                        }
                    }    
                    ...
                    if (doujinveza.error) {
                        ...
                        tobz.sendText(from, captions, id)
                        tobz.sendFileFromUrl(from, shortenernh, `${title}.zip`, '_NHENTAI DOWNLOADER BY TOBZ_', id)
                        //await tobz.sendFileFromUrl(from, result, `${title}.pdf`, `*NHENTAI BY TOBZ*`, id).catch(() => tobz.reply(from, mess.error.Yt4, id))
                        //await limitAdd(serial)
                   }
                }
            } catch (err) {
        ...
        case prefix+'xvideos': // SEARCH VIDEO FROM XVIDEOS
            ...
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#xvideos* [ Lagu ]`)
		...
		case prefix+'xxx':
            ...
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#xxx* [ Judul ]`)
        ...
        case prefix+'getvideo': // DOWNLOADER VIDEO FROM #VIDEO
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *#getvideo* [IdDownload], untuk contoh silahkan kirim perintah *#readme*', id)
            try {    
            if (quotedMsg && quotedMsg.type == 'image') {
                ...
                if (args.length === 1) return tobz.reply(from, 'Kirim perintah *#getmusik [IdDownload]*, untuk contoh silahkan kirim perintah *#readme*')
	    ...
	    case prefix+'neon': {
                                                        if(isReg(obj)) return
                                                        if(cekumur(cekage)) return
														const neoncuys = body.slice(6)
                                                        const neoncuys1 = neoncuys.split('|')[0]
                                                        const neoncuys2 = neoncuys.split('|')[1]
                                                        const neoncuys3 = neoncuys.split('|')[2]
                                                        if (!neoncuys) return tobz.reply(from, 'Kirim perintah #neon text1|text2|text3\n\nContoh : #neon Follow|Instagram|Nabilazaaaa_', id)
                                                        tobz.reply(from, mess.wait, id)
		...
		case prefix+'wolf': {
                                                        ...
                                                        const neoncuys1 = neoncuys.split('|')[0]
                                                        const neoncuys2 = neoncuys.split('|')[1]
                                                        if (!neoncuys) return tobz.reply(from, 'Kirim perintah #wolf text1|text2\n\nContoh : #neon Bigetron|Esport', id)
                                                        tobz.reply(from, mess.wait, id)
                                                        const puppeteer = require('puppeteer')
                                                try {
                                                    (async () => {
		...
		case prefix+'epep': {
                                                        if(isReg(obj)) return
                                                         if(cekumur(cekage)) return 
														const neoncuyss = body.slice(6)
                                                        const neoncuys11 = neoncuyss.split('|')[0]
                                                        const neoncuys22 = neoncuyss.split('|')[1]
                                                        if (!neoncuyss) return tobz.reply(from, 'Kirim perintah #epep text1|text2\n\nContoh : #epep Bigetron|Esport', id)
                                                        tobz.reply(from, mess.wait, id)
                                                        const puppeteer = require('puppeteer')
                                                try {
                                                    (async () => {
                                                        ...
                                                        await page
                                                            ...
                                                            .then(async () => {
                                                                await page.type("#text-0", neoncuys11);
                                                                await page.type("#text-1", neoncuys22);
                                                                await page.click("#submit");
                                                                await new Promise(resolve => setTimeout(resolve, 3000));
                                                                const element = await page.$(
                                                                    'div[class="thumbnail"] > img'
		...
		case prefix+'glitch': {
                                                        if(isReg(obj)) return
                                                        if(cekumur(cekage)) return
														const neoncuysss = body.slice(8)
                                                        const neoncuys11q = neoncuysss.split('|')[0]
                                                        const neoncuys22q = neoncuysss.split('|')[1]
                                                        if (!neoncuysss) return tobz.reply(from, 'Kirim perintah #glitch text1|text2\n\nContoh : #glitch Nabil|Mazaa', id)
                                                        tobz.reply(from, mess.wait, id)
                                                        const puppeteer = require('puppeteer')
                                                try {
                                                    (async () => {
                                                        ...
                                                        await page
                                                            ...
                                                            .then(async () => {
                                                                await page.type("#text-0", neoncuys11q);
                                                                await page.type("#text-1", neoncuys22q);
                                                                await page.click("#submit");
                                                                await new Promise(resolve => setTimeout(resolve, 3000));
                                                                const element = await page.$(
                                                                    'div[class="thumbnail"] > img'
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Nabil|Dev HNZM*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                tobz.reply(from, mess.wait, id)
                const glitch1 = argz[1]
                ...
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Tobz|Dev HNZM*`, id)
            }
            break
		case prefix+'anjay1': {
                                                        if(isReg(obj)) return
                                                        if(cekumur(cekage)) return
														const neoncuysso = body.slice(8)
                                                        const neoncuys11t = neoncuysso.split('|')[0]
                                                        const neoncuys22t = neoncuysso.split('|')[1]
                                                        if (!neoncuysso) return tobz.reply(from, 'Kirim perintah #marvel text1|text2\n\nContoh : #marvel Nabil|Mazaa', id)
                                                        tobz.reply(from, mess.wait, id)
                                                        const puppeteer = require('puppeteer')
                                                try {
                                                    (async () => {
                                                        ...
                                                        await page
                                                            ...
                                                            .then(async () => {
                                                                await page.type("#text-0", neoncuys11t);
                                                                await page.type("#text-1", neoncuys22t);
                                                                await page.click("#submit");
                                                                await new Promise(resolve => setTimeout(resolve, 3000));
                                                                const element = await page.$(
                                                                    'div[class="thumbnail"] > img'
		...
		case prefix+'anjay2': {
                                                        if(isReg(obj)) return
                                                        if(cekumur(cekage)) return
														const neoncuyssoy = body.slice(7)
                                                        const neoncuys11tt = neoncuyssoy.split('|')[0]
                                                        const neoncuys22tt = neoncuyssoy.split('|')[1]
                                                        if (!neoncuyssoy) return tobz.reply(from, 'Kirim perintah #marvel2 text1|text2\n\nContoh : #marvel2 Nabil|Mazaa', id)
                                                        tobz.reply(from, mess.wait, id)
                                                        const puppeteer = require('puppeteer')
                                                try {
                                                    (async () => {
                                                        ...
                                                        const page = await browser.newPage();
                                                        await page
                                                            .goto("https://textpro.me/create-logo-style-marvel-studios-ver-metal-972.html", {
                                                                waitUntil: "networkidle2"
                                                            })
                                                            .then(async () => {
                                                                await page.type("#text-0", neoncuys11tt);
                                                                await page.type("#text-1", neoncuys22tt);
                                                                await page.click("#submit");
                                                                await new Promise(resolve => setTimeout(resolve, 3000));
                                                                const element = await page.$(
                                                                    'div[class="thumbnail"] > img'
        ...
        case prefix+'youtubesearch': // SEARCH YOUTUBE
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *#youtubesearch [query]*, Contoh : #youtubesearch alan walker alone')
		...
		case prefix+'brainlysearch':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *#brainlysearch [query]*, Contoh : *#brainlysearch Siapa itu HNZM?*')
            const bser = body.slice(15)
            tobz.reply(from, mess.wait, id)
            try {
                const bser2 = await axios.get(`https://api.vhtear.com/brainly?query=${bser}&apikey=${vhtearkey}`)
                const bser3 = bser2.data.result
                 let keluarplay = `*Hasil Pencarian : ${bser}*\n`
                for (let i = 0; i < bser3.result.length; i++) {
                    keluarplay += `\n═════════════════\n\n*Judul* : ${bser3.result[i].title}\n*Link* : ${bser3.result[i].url}\n`
                }
                await tobz.reply(from, keluarplay, id)
            }   catch (err){
                console.log(err)
            }
            break
        case prefix+'shopee': // SEARCH SHOPEE PRODUCT
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *#shopee [query]*, Contoh : *#shopee HP Samsul a20*')
        ...
        case prefix+'playstore': // SEARCH PLAYSTORE
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *#playstore [query]*, Contoh : *#playstore Mobile Legends*')
...
*Text : ${swrt}*`
		 ...
		 case prefix+'sarah':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *#sarah [query]*\nContoh : *#sarah ZeroTwo*', id)
            const txtsarah = body.slice(7)
            const sapi = await sarahfs(txtsarah)
            await tobz.sendFile(from, sapi, 'ss.jpg', 'cekrek', id)
            .catch(() => {
            ...
            case prefix+'tahta':
                ...
                await tobz.sendFileFromUrl(from, `https://api.vhtear.com/hartatahta?text=${jreng}&apikey=ANTIGRATISNIHANJENKKK`,`${jreng}.jpg`,`*Harta*\n*Tahta*\n*${jreng}*`, id)        
                await limitAdd(serial)
                break
...
➸ *Total Rekaman:* ${recording}`
        ...
        case prefix+'twitter':
            ...
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
            let mapz = await slicedArgs.join(' ')
            console.log(mapz)
            try {
          ...
          break
          case prefix+`movie`:
                   // if (!isRegis) return tobz.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user SELF BOT, untuk pendaftaran bisa menggunakan !regis |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                  //  if (!isAdmin) return tobz.reply(from, 'Mohon maaf anda tidak bisa menggunakan fitur premium!', id)
                    ...
                    try {
                        ...
                        tobz.sendFileFromUrl(from, thumb, 'thumbjpg', movi, id)
                    } catch (err) {
                        tobz.sendText(ownerNumber, 'Error Movie : ' + err)
                        tobz.reply(from, 'Error!', id)
                    }
                    break
		case prefix+'addlimit':
			if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner Davbot!', id)
			var found = false;
                    Object.keys(limit).forEach((i) => {
                    ...
                    if (found !== false) {
						...
						tobz.sendTextWithMentions(from, `menambahkan ${args[1]} limit ke @${mentionedJidList[0].pengirim('@c.us', '')}` )
					break
                    case prefix+'gift': // Hanya Admin & Owner HNZM yang bisa gift Limit
                    if (!isAdmin, !isOwner) return tobz.reply(from, `Maaf, perintah ini hanya dapat dilakukan oleh Admin HNZM!`, id)
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'jadwaltv':
            ...
            const jadwal = await axios.get('http://api-melodicxt.herokuapp.com/api/jadwaltv?query=antv&apiKey=administrator')
        ...
        case prefix+'jadwaltvnow': // API MATI
            ...
            await limitAdd(serial)
            const jadwalNow = await axios.get('http://api-melodicxt.herokuapp.com/api/jadwaltvnow?apiKey=administrator')
        ...
        case prefix+'puisi': // ARUGAZ
            ...
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            const puisi = await axios.get('https://arugaz.herokuapp.com/api/puisi1').json()
        ...
        case prefix+'puisi2': // ARUGAZ
            ...
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            const puisi2 = await axios.get('https://arugaz.herokuapp.com/api/puisi2').json()
        ...
        case prefix+'puisi3': // ARUGAZ
            ...
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            const puisi3 = await axios.get('https://arugaz.herokuapp.com/api/puisi3').json()
		...
		case prefix+'slap':
            ...
            await tobz.sendGiphyAsSticker(from, 'https://media.giphy.com/media/S8507sBJm1598XnsgD/source.gif')
            tobz.sendTextWithMentions(from, '@' + jejiik + ' *slapped* ' + arg[1])
            break
        case prefix+'hug':
		    if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            arg = body.trim().split(' ')
            ...
            tobz.sendTextWithMentions(from, '@' + janjing + ' *peyuuuk* ' + arg[1])
            break
        case prefix+'nye':
		    if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            arg = body.trim().split('')
            ...
            tobz.sendTextWithMentions(from, '@' + jancuk7 +' *nye nye* ' + arg[1])
            break
        case prefix+'pat':
		    if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            arg = body.trim().split(' ')
            ...
            tobz.sendTextWithMentions(from, '@' + jartod + ' *👈 Si Mengelu-elus si👉* ' + arg[1])
            break
	    case prefix+'trigger':
                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
                    arg = body.trim().split(' ')
                    ...
                    await tobz.sendGiphyAsSticker(from, kiya, id)
                    tobz.sendTextWithMentions(from, '@' + bruhh + ' *Triggered* ' + arg[1])
                    break
        // ADMIN & OWNER
        case 'cekprefix':
        case 'prefix':
            tobz.reply(from, `PREFIX YANG SAAT INI DIGUNAKAN *「* ${prefix} *」*\nPREFIX ADALAH PERINTAH DI AWAL,CONTOH ${prefix}menu`, id)
            break
        ...
        case prefix+'bc': // KASIH CREDIT DONG KALO COPAS
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner HNZM`, id)
                bctxt = body.slice(4)
                txtbc = `*「 HNZM BROADCAST 」*\n\n${bctxt}`
                ...
                break
		case prefix+'infoall':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin HNZM', id)
            ...
            const textInfo = body.slice(9)
            const groupMem = await tobz.getGroupMembers(groupId)
            let hehexX = `╔══✪〘 Mention All 〙✪══\n\n`
            for (let i = 0; i < groupMem.length; i++) {
                hehexX += '╠➥'
                hehexX += ` @${groupMem[i].id.replace(/@c.us/g, '')}\n`
            }
            hehexX += '╚═〘 *M U R Z E M E  B O T* 〙'
            await tobz.sendTextWithMentions(from, `Info dari : ${pushname}\n` + textInfo+ '\n\n' +hehexX)
            break
        ...
        case prefix+'ownergroup':
            ...
            break
        case prefix+'otagall': // FOR OWNER & ADMIN HNZM
        case prefix+'omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner HNZM', id)
            ...
            case prefix+'linkmem':
                if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
                const groupMem5 = await tobz.getGroupMembers(groupId)
                let hehe5 = '╔══✪〘 Link Nomor Member 〙✪══\n'
            for (let i = 0; i < groupMem5.length; i++) {
                    hehe5 += ` https://wa.me/${groupMem5[i].id.replace(/@c.us/g, '').replace(/@/g, '')}\n`
                }
                hehe5 += '╚═〘 kentod BOT 〙'
                await tobz.sendText(from, hehe5)
                tobz.reply(from, `Total Member : ${groupMem5.length}`, id)
                tobz.reply(from, 'Get All Link Member Success!', id)
                break
        case prefix+'tagall': // FOR GROUP ADMINS
        case prefix+'mentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
            const groupMeme = await tobz.getGroupMembers(groupId)
            let hehe = '╔══✪〘 Mention All 〙✪══\n'
            for (let i = 0; i < groupMeme.length; i++) {
                hehe += '╠➥'
                hehe += ` @${groupMeme[i].id.replace(/@c.us/g, '')}\n`
            }
            hehe += '╚═〘 HNZM BOT 〙'
            await sleep(2000)
        ...
        case prefix+'oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin HNZM', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case prefix+'oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin HNZM', id)
            await tobz.sendText(from,'HNZM DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case prefix+'leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case prefix+'opromote':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin HNZM', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Fitur ini hanya bisa di gunakan ketika bot menjadi admin', id)
            if (mentionedJidList.length === 0) return tobz.reply(from, 'Untuk menggunakan fitur ini, kirim perintah *opromote* @tagmember', id)
        ...
        case prefix+'join':
            ...
            if (check.status === 200) {
                await tobz.joinGroupViaLink(link).then(() => tobz.reply(from, 'Bot akan segera masuk!', id))
            } else {
        ...
        case prefix+'sider':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)                
            if (!quotedMsg) return tobz.reply(from, `Tolong Reply Pesan HNZM`, id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, `Tolong Reply Pesan HNZM`, id)
            try {
                ...
                console.log(err)
                tobz.reply(from, `Maaf, Belum Ada Yang Membaca Pesan HNZM atau Mereka Menonaktifkan Read Receipts`, id)    
            }
            break
		case prefix+`say`:
		case prefix+`bacot`:
            //if (!isDaftar) return tobz.reply(from, `Maaf kak ${pushname} kamu blom terdaftar\nJika ingin mendaftar ketik\n*${prefix}daftar add nomoranda*\nContoh : ${prefix}daftar add 6289523258649`, id)
            if (!isReg) return tobz.reply(from, `Maaf kak ${pushname} kamu blom terdaftar\nJika ingin mendaftar ketik\n*${prefix}daftar add nomoranda*\nContoh : ${prefix}daftar add 6289523258649`, id)
			if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #ceklimit Untuk Mengecek Kuota Limit Kamu`, id)
		...
		case prefix+`adds`:
            //if (!isDaftar) return tobz.reply(from, `Maaf kak ${pushname} kamu blom terdaftar\nJika ingin mendaftar ketik\n*${prefix}daftar add nomoranda*\nContoh : ${prefix}daftar add 6289523258649`, id)
			if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (args.length == 1) return tobz.reply(from, `Kirim perintah #addsay [teks]\ncontoh #addsay atas gw gay`, id)
            const says = body.slice(8)
            say.push(says)
            fs.writeFileSync('./lib/database/say.json', JSON.stringify(say))
            tobz.reply(from, `sukses menambahkan ke database!`,id)
            break
        case prefix+`delsay`:
		case prefix+`dels`:
            //if (!isDaftar) return tobz.reply(from, `Maaf kak ${pushname} kamu blom terdaftar\nJika ingin mendaftar ketik\n*${prefix}daftar add nomoranda*\nContoh : ${prefix}daftar add 6289523258649`, id)
			if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (args.length == 1) return tobz.reply(from, `Kirim perintah #delsay [teks], contoh #delsay atas gw gay`, id)
            const sayso = body.slice(8)
            let delsayso = say.indexOf(sayso)
            say.splice(delsayso, 1)
            fs.writeFileSync('./lib/database/say.json', JSON.stringify(say))
            tobz.reply(from, `sukses menghapus dari database`,id)
            break
        case prefix+`saylist`:
		case prefix+`listsay`:
		case prefix+`listbct`:
            //if (!isDaftar) return tobz.reply(from, `Maaf kak ${pushname} kamu blom terdaftar\nJika ingin mendaftar ketik\n*${prefix}daftar add nomoranda*\nContoh : ${prefix}daftar add 6289523258649`, id)
			let saylisto = `List Bacotan Autis\nTotal : ${say.length}\n`
            for (let i of say) {
            saylisto += `☛ ${i}\n`
            }
		...
		case prefix+`premilist`:
      	  case prefix+`listpremi`:
                  let premilur = `┌──「 𝙇𝙄𝙎𝙏 𝙐𝙎𝙀𝙍 𝙋𝙍𝙀𝙈𝙄𝙐𝙈 」\n│\n├> Total : ${premi.length}\n`
                  for (let i of premi) {
                  premilur += `☛ ${i.replace(/@c.us/g,'')}\n`
                  }
                  await tobz.reply(from, premilur, id)
                  break
          case prefix+`premi`:
               if (!isOwner) return tobz.reply(from, 'JUST OWNER', id)
               if (args.length == 1) return tobz.reply(from, `lah lah`, id)
               if (args[1] == 'add') {
                   premi.push(args[2] + '@c.us')
                   fs.writeFileSync('./lib/premi.json', JSON.stringify(premi))
                   tobz.reply(from, 'Menambahkan 1 premium user')
               } else
               if (args[1] == 'del') {
                   let uds = premi.indexOf(args[2] + '@c.us')
                   premi.splice(uds, 1)
                   fs.writeFileSync('./lib/premi.json', JSON.stringify(premi))
                   tobz.reply(from, 'Menghapus 1 premium user')
               } else {
                   for (let i = 0; i < mentionedJidList.length; i++) {
                       premi.push(mentionedJidList[i])
                       fs.writeFileSync('./lib/premi.json', JSON.stringify(premi))
                       tobz.reply(from, 'Menambahkan 1 premium user', id)
                   }
               }
                break
	  ...
	  case prefix+'edotensei':
            ...
            for (let i = 0; i < mentionedJidList.length; i++) {
				...
				tobz.reply(from, 'Romaji Khuciyose Edo Tensei🙏', id)
                await tobz.removeParticipant(groupId, mentionedJidList[i])
                await sleep(3000)
            ...
            case prefix+'panggil':
		if (!isReg) return tobz.reply(from, `Maaf kak ${pushname} kamu blom terdaftar\nJika ingin mendaftar ketik\n*${prefix}daftar add nomoranda*\nContoh : ${prefix}daftar add 6289523258649`, id)
        if (!isGroupAdmins) return tobz.reply(from, ' Perintah ini hanya bisa dilakukan oleh admin group',id)
        arg = body.trim().split(' ')
            if (args.length >= 2) {
                let jumlahh = arg[2]
                let orangg = arg[3]
            for(var i=1;i<=jumlahh;i++){
                await tobz.sendTextWithMentions(from, `Admin @${sender.id} memanggil ${orangg} ${i}x`)
            }}
            break
        case prefix+'getses':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner HNZM', id)            
        ...
        case prefix+'addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner HNZM!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin HNZM!', id)
                }
            break
        case prefix+'deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner HNZM!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin HNZM!', id)
            break
        case prefix+'block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner HNZM!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
	 ...
	 case '#you':
        ...
        if (isGroupMsg) {
            ...
            await tobz.sendFileFromUrl(from, pfp, 'pfp.jpg', `*PROFILE :* \n☛ *User: ${namae}* \n☛ *Info: ${status}* \n☛ *Block : ${block}* \n☛ *Banned : ${bend}* \n☛ *Admin Group: ${adm}* \n☛ *Admin HNZM: ${donate}*`)
         } if (quotedMsg) {
        ...
        case prefix+'ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin HNZM!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin HNZM!`, id)
		...
		case prefix+`hilih`:
				   ...
				   if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                    ...
                    if (quotedMsg) {
                        ...
                        const skyaaara = await axios.get('https://freerestapi.herokuapp.com/api/v1/hilih?kata=' + quoteText).json()
                        tobz.reply(from, skyaaara.result, id)
                    } else {
                        const daasda = body.slice(7)
                        const skyaaara = await axios.get('https://freerestapi.herokuapp.com/api/v1/hilih?kata=' + daasda).json()
                        tobz.reply(from, skyaaara.result, id)
                    }
                    break
			case prefix+`ptlvid`:
				   ...
				   if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                    ...
                    tobz.sendFileFromUrl(from, rindKiya, 'asupan.mp4', 'Nih', id)
                    break
			 case prefix+`indohot`:
                   if(isReg(obj)) return
                   if(cekumur(cekage)) return
				   if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                    ...
                    await limitAdd(serial)
                    if (isGroupMsg) {
                        if (!isNsfw) return tobz.reply(from, 'Command/Perintah NSFW belum diaktifkan di group ini!', id)
                        const dsa = await axios.get(`https://arugaz.herokuapp.com/api/indohot`).json()
                        const {
                            genre,
                            judul,
                            url,
                        ...
                        await tobz.sendText(from, `*Judul* : ${judul}\n*Durasi* : ${durasi}\n*Genre* : ${genre}\n*Negara* : ${country}\n*Link* : ${url}`, id)
                    }
                    break
                    case prefix+'tomp3':
                        if(isLimit(serial)) return
                        if ((isMedia || isQuotedVideo || isQuotedFile)) {
                            ...
                            fs.writeFile(fileInputPath, mediaData, err => {
                                if (err) return tobz.sendText(from, 'Ada yang error saat menulis file\n\n' + err) && _err(err)
		...
		case prefix+'bass':{
                   if(isLimit(serial)) return
                    if (isQuotedAudio) {
                        ...
                        tobz.reply(from, 'tunggu ya sedang diproses', ('mp3', 'mp3', `Bass ${freq}hz: +${dB}dB`), id)
                        fs.writeFile(fileInputPath, mediaData, err => {
                            if (err) return tobz.sendText(from, 'Ada yang error saat menulis file', id)
                            ffmpeg(fileInputPath)
                                ...
                                .on('end', function () {
                                    ...
                                    // fs.readFile(fileOutputPath, { encoding: 'base64' }, (err, base64) => {
                                    // if (err) return tobz.sendText(from, 'Ada yang error saat membaca file .mp3') && console.log(color('[ERROR]', 'red'), err)
                                    tobz.sendFile(from, fileOutputPath, 'distorted.mp3', '', id)
                                    // })
                                    setTimeout(() => {
                                        try {
                                            fs.unlinkSync(fileInputPath)
                ...
                case prefix+'fitnah':	
				case prefix+'fake':          
if (!isGroup) tobz.reply(mess.only.group)
arg = body.substring(body.indexOf(' ') + 1)
				isi = arg.split(' |')[0] 
				pesan = arg.split('|')[1] 
				pesan2 = arg.split('|')[2] 
tobz.sendPtt(pesan, isi, pesan2)
break
                case 'iri':
  tobz.sendPtt(from, './media/iri.mp3', id)
break
case 'tapi':
  tobz.sendPtt(from, './media/tapi.mp3', id)
break
case 'tts':
  tobz.sendPtt(from, './media/tts.mp3', id)
break
case 'mantap':
  tobz.sendPtt(from, './media/mantap.mp3', id)
break
case 'kang':
  tobz.sendPtt(from, './media/kang.mp3', id)
break
case 'sasageyo':
  tobz.sendPtt(from, './media/sasageyo.mp3', id)
break
case 'sory bang jago':
  tobz.sendPtt(from, './media/sory bang jago.mp3', id)
break
case 'abang':
  tobz.sendPtt(from, './media/abang.mp3', id)
break
case prefix+'linkgroup':
				...
				case prefix+'linkgc':
					...
					linkgc = await tobz.groupInviteCode (from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nNih Link Grup ${groupName}`
					tobz.sendMessage(from, yeh, text, {quoted: mek})
					break
case 'p':
        case 'halo':
		case '.':
            tobz.reply(from, `daripada gabut mending ketik *#help* untuk menampilkan menu`, id)
            break
        case 'assalamualaikum':
		case 'asalamualaikum':
        case 'samlekom':
            tobz.reply(from, `Walaikumsalam,ada yg bisa saya bantu ?,ketik *#help* untuk menampilkan menu`, id)
            break
        case 'Hnzm':tobz.reply(from, `Aku di sini kak ${pushname} ? ketik *#help* untuk menampilkan menu`, id)
        break
        case 'bot':
            tobz.reply(from, `Aku di sini kak ${pushname} ? ketik *#help* untuk menampilkan menu`, id)
            break
        // LIST MENU
        case prefix+'menu':
        case prefix+'help':
            ...
            break
        case prefix+'HNZMgroup':
        case prefix+'botgroup':
        case prefix+'groupbot':
            tobz.reply(from, `Link Group WHATSAPP HNZM : https://chat.whatsapp.com/IWsd1WHSvp6KdLRXKIQYGq\nJangan Lupa Join Ya Kak ${pushname} / Link Group Telegram HNZM : t.me/HNZMBOTOFFCIIAL\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        case prefix+'groupmenu':
            if(isReg(obj)) return
			tobz.sendText(from, groupcmd)
            break
		case prefix+'funadmmenu':
		case prefix+'adminfunmenu':
		case prefix+'menuadminfun':
		case prefix+'funadminmenu':
		     if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin HNZM!`, id)
		     tobz.sendText(from, premiumcmd)
			 break
        ...
        case prefix+'adminmenu':
            if(isReg(obj)) return
			if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin HNZM', id)
		...
		case 'runtime':
            ...
            tobz.reply(from, `HNZM telah berjalan selama :\n*${format(uptime)}*`, id)
            break
        // INFORMATION
        case prefix+'donate':
...
// By Gimenz
        case prefix+'snk':
            ...
            break
        default:
            //if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup HNZM, Link Ada Di Bio atau Bisa Mengetik #botgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
